...
@discohook.ApplicationCommand.slash(description="ask me", options = options)
async def ask_command(interaction, prompt: str):

  # await interaction.defer() # because it takes more than 3 seconds!
  await interaction.response.send(prompt + "... ask it...?")
